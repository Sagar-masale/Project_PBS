...
const OrderBill = ({ selectedOrder, handleClose, invoiceRef }) => {
  ...
  useEffect(() => {
    ...
    switch (selectedOrder.orderStatus) {
      case "Pending":
        setOrderStatusInfo(
          "Thank you for your order! Your order is currently pending and being processed. We will notify you once it is confirmed."
        );
        setOrderStatusTitle("Your Order is Being Processed");
        break;
      case "Canceled":
        setOrderStatusInfo(
          "We regret to inform you that your order has been canceled. If this was unintentional, please contact our support team."
        );
        setOrderStatusTitle("Your Order Has Been Canceled");
        break;
      case "Success":
        setOrderStatusInfo(
          "Thank you! Your purchase has been successfully processed. Please find the receipt below."
        );
  ...
  const calculateTotalFinalPrice = () => {
   return selectedOrder?.products?.reduce((acc, product) => {
    return acc + (product?.price || 0);
  }, 0);
  };

  const totalOrderPrice = calculateTotalFinalPrice();
  ...
  return (
    ...
    >
      ...
      <div className="mt-6">
        <h2 className="text-xl font-semibold text-[#432a58]">
          #{orderStatusTitle}
        </h2>
        ...
        <div className="mt-8">
          ...
          <div className="flex flex-col gap-6">
            {selectedOrder.orderDetails.map((order, index) => {
              ...
              const productRoot = selectedOrder;
              if (!product) return null;

              const quantity = product.orderQuantity || 1;
              const price = product.orderedProductPrice;
              const productWeight = product.orderProductWeight || 0;
              const metalProductPrice = product.orderedProductPrice || 0;
              const mackingChargesPerGram = product.mackingCharges || 0;

              const totalFinalPrice = (metalProductPrice + mackingChargesPerGram);
                ...
                >
                  <div className="flex gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        Qty: {quantity} | Gram: {productWeight}
                      </p>
                      <p className="text-xs text-gray-500">
                        Dec: {order.ProductDescription}
                      </p>
                      <p className="text-xs font-medium">Base Price: ₹{price} per gram</p>
                    </div>
                  </div>
                  <p className="text-sm font-semibold text-gray-800 text-right sm:text-left">
                    ₹ {totalFinalPrice * productWeight.toFixed(2) * quantity}
                  </p>
                </div>
              );
            })}
          ...
          <div className="mt-6 text-right space-y-1 border-t pt-4">
            <p className="text-sm text-gray-700">
              Total: <span className="font-semibold">₹{selectedOrder.totalAmount.toFixed(2)}</span>
            </p>
            <p className="text-sm text-gray-700">
              Discount: <span className="font-semibold text-green-600">₹{(selectedOrder.discount || 0).toFixed(2)}</span>
            </p>
            <p className="text-lg font-bold text-gray-900">
              Grand Total: ₹{(selectedOrder.totalAmountWithDiscount.toFixed(2))}
            </p>
          </div>
        </div>
      ) : (
